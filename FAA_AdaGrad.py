...
# Gradient descent with adagrad with minibatch of size 100
list_err = []
list_err_test = []
list_iteration = []
...
for i in range(20000):
    ...
    if i%100 == 0:
        ...
        y_p=tf.argmax(y_out, 1)
        acuracy_test,y_pred = sess.run([accuracy,y_p], feed_dict={x: mnist.test.images, y_: mnist.test.labels})
        print "test accuracy %g"%(acuracy_test)
...
#vue graphique
pylab.plot(list_iteration, list_err,"b")
pylab.plot(list_iteration, list_err_test,"r")
...
cax = ax.matshow(cm)
plt.title('Matrice de confusion sans normalisation')
fig.colorbar(cax)
